...
class RSSGenerator:
    ...
    def __init__(self, schemas_path: Path, git_repo: Optional[Path] = None):
        ...
        Args:
            ...
            git_repo: Optional path to git repository for commit information
        """
        self.schemas_path = schemas_path
        self.git_repo = git_repo
        self.schemas_data: Dict[str, Any] = {}
        self._load_schemas_data()

    def _load_schemas_data(self) -> None:
        """Load schemas data with graceful error handling."""
        if not self.schemas_path.exists():
            logger.warning(f"⚠️  Schemas file not found: {self.schemas_path}")
